...
export async function withInteractiveScreen(
  ...
  const cleanup = () => {
    if (cleaned) return;
    cleaned = true;
    process.stdout.write(`${RESET_ATTRIBUTES}${DISABLE_BRACKETED_PASTE}${DISABLE_MOUSE}${SHOW_CURSOR}${setTerminalTitle("Terminal")}${EXIT_ALT_SCREEN}`);
  };
  ...
  process.stdout.write(`${DISABLE_MOUSE}${DISABLE_BRACKETED_PASTE}${title}${ENTER_ALT_SCREEN}${RESET_ATTRIBUTES}${CLEAR_SCREEN}${ENABLE_MOUSE}${ENABLE_BRACKETED_PASTE}${HIDE_CURSOR}`);
